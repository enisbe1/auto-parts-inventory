...
function NewPartContent() {
  ...
  // Price inheritance: debounced search when name changes
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        const { data } = await api.get('/parts', { params: { search: name.trim(), limit: 10, page: 1 } });
        const results: Part[] = data.data ?? data ?? [];
        // Find a matching part (same name, case-insensitive) that has a price set
        ...
        if (match && match.price != null) {
          ...
          const vehicleLabel = match.vehicle?.variant?.generation?.model?.make?.name
            ? [
                match.vehicle.variant.generation.model.make.name,
                match.vehicle.variant.generation.model.name,
                match.vehicle.year ? `(${match.vehicle.year})` : '',
              ].filter(Boolean).join(' ')
            : match.vehicle ? `Vehicle #${match.vehicle.id}` : 'another vehicle';
          setSuggestedFrom(vehicleLabel);
        }
      } catch {
        // silently ignore
      }
  ...
  const sel = 'w-full bg-[#18181b] border border-[#27272a] text-zinc-100 focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500/40 rounded-xl px-4 py-2.5 text-sm transition';
    ...
    <div className="p-8 max-w-2xl mx-auto">
      ...
      <div className="mb-8">
        ...
        >
          ...
          {preselectedVehicleId ? t.nav.vehicles : t.nav.parts}
        </button>
        <h1 className="text-2xl font-bold text-zinc-100">{t.newPart.title}</h1>
        <p className="text-zinc-400 text-sm mt-1">{t.newPart.subtitle}</p>
      </div>
      ...
      <form onSubmit={submit} className="space-y-6">
        ...
        <div className="bg-[#111113] border border-[#27272a] rounded-xl p-6 space-y-4">
              ...
              <div className="mt-2 flex items-center justify-between bg-blue-500/10 border border-blue-500/20 rounded-xl px-4 py-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-blue-400 font-medium">
                    {t.newPart.priceSuggestion(suggestedFrom)} <span className="font-bold">€{suggestedPrice.toFixed(2)}</span>
                  </span>
                </div>
                <button
                  type="button"
                  onClick={applySuggestedPrice}
                  className="ml-3 shrink-0 text-xs font-semibold text-blue-400 bg-blue-500/10 border border-blue-500/20 px-3 py-1.5 rounded-lg hover:bg-blue-500/20 transition-colors"
                >
                  {t.newPart.applyPrice}
                </button>
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-zinc-400 mb-1.5">{t.newPart.partNumber}</label>
              <input value={partNumber} onChange={e => setPartNumber(e.target.value)} placeholder="OEM or aftermarket no." className={inp} />
            </div>
            <div>
              <label className="block text-sm font-medium text-zinc-400 mb-1.5">{t.newPart.price}</label>
              <div className="relative">
                ...
                <input
                  ...
                  min={0}
                  step="0.01"
                  className="w-full bg-[#18181b] border border-[#27272a] text-zinc-100 placeholder-zinc-700 focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500/40 rounded-xl pl-8 pr-4 py-2.5 text-sm transition"
                />
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-zinc-400 mb-1.5">{t.newPart.condition}</label>
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-400 mb-1.5">{t.newPart.status}</label>
        ...
        <div className="bg-[#111113] border border-[#27272a] rounded-xl p-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-zinc-400 mb-1.5">{t.newPart.vehicle}</label>
            ...
            {vehicles.length === 0 && (
              <p className="text-xs text-zinc-600 mt-1.5">No vehicles in inventory yet — <a href="/vehicles/new" className="text-blue-400 hover:underline">add one first</a></p>
            )}
          </div>

          <div>
            <label className="block text-sm font-medium text-zinc-400 mb-1.5">{t.newPart.category}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-zinc-400 mb-1.5">{t.newPart.notes}</label>
            <textarea
              value={notes}
        ...
        <div className="flex gap-3">
          ...
          >
            ...
            {loading ? t.newPart.submitting : t.newPart.submit}
          </button>
        </div>
      </form>
    </div>
