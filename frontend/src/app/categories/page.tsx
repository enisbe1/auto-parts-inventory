...
export default function CategoriesPage() {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-[var(--text-primary)]">{t.categories.title}</h1>
        <p className="text-[var(--text-secondary)] text-sm mt-1">{t.categories.subtitle}</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
        {/* Add form */}
        <div className="md:col-span-2">
          <div className="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-6 sticky top-8 shadow-xl shadow-black/20">
            <h2 className="font-semibold text-[var(--text-primary)] mb-1">{t.categories.addCategory}</h2>
            <p className="text-xs text-[var(--text-muted)] mb-5">New categories will appear in the parts form</p>
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1.5">{t.categories.description}</label>
                <input
                  value={description}
        ...
        <div className="md:col-span-3">
          <div className="bg-[var(--surface)] border border-[var(--border)] rounded-xl overflow-hidden shadow-xl shadow-black/20">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border)] bg-[var(--surface)]">
              <span className="font-semibold text-[var(--text-primary)]">All Categories</span>
              <span className="text-xs font-medium text-[var(--text-secondary)] bg-[var(--surface-raised)] px-2 py-0.5 rounded-full border border-[var(--border)]">
                {categories.length}
              </span>
            </div>
            ...
            {categories.length === 0 ? (
              <div className="px-6 py-12 text-center">
                <div className="w-10 h-10 bg-[var(--surface-raised)] rounded-xl flex items-center justify-center mx-auto mb-3">
                ...
                <p className="text-sm text-[var(--text-secondary)]">{t.categories.noCategories}</p>
              </div>
            ) : (
                ...
                {categories.map((c) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="text-xs font-medium text-[var(--text-muted)] hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100"
                    >
                      {t.common.delete}
                    </button>
                  </li>
