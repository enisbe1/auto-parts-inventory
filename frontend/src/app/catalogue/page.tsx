...
export default function CataloguePage() {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-[var(--text-primary)]">{t.catalogue.title}</h1>
        <p className="text-[var(--text-secondary)] text-sm mt-1">{t.catalogue.subtitle}</p>
        {/* Breadcrumb */}
        <div className="flex items-center gap-1.5 text-sm mt-3">
          <span className={selMake ? "text-[var(--text-secondary)]" : "font-medium text-zinc-300"}>
            {t.catalogue.makes}
          </span>
          {selMake && (
            <>
              <svg className="w-3 h-3 text-[var(--text-muted)]" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
              </svg>
              <span className={selModel ? "text-[var(--text-secondary)]" : "font-medium text-zinc-300"}>{selMake.name}</span>
            </>
          )}
          {selModel && (
            <>
              ...
              <span className={selGen ? "text-[var(--text-secondary)]" : "font-medium text-zinc-300"}>{selModel.name}</span>
            </>
          )}
          {selGen && (
            <>
