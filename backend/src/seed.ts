...
async function seed() {
  ...
  const categoryData = [
    {
      name: 'Engine',
      description:
        'Engine block, head, pistons, camshaft, crankshaft, timing components',
    },
    {
      name: 'Gearbox',
      description:
        'Manual and automatic transmission assemblies and components',
    },
    {
      name: 'Suspension',
      description:
        'Control arms, struts, springs, stabiliser bars, wheel bearings',
    },
    {
      name: 'Brakes',
      description: 'Callipers, discs, pads, master cylinders, brake lines',
    },
    {
      name: 'Body',
      description: 'Doors, bonnets, bumpers, wings, boot lids, glass panels',
    },
    {
      name: 'Electronics',
      description:
        'ECU, sensors, wiring harnesses, alternators, starters, instrument clusters',
    },
    {
      name: 'Interior',
      description:
        'Seats, dashboards, door cards, carpets, steering wheels, mirrors',
    },
    {
      name: 'Cooling',
      description: 'Radiators, water pumps, thermostats, fans, intercoolers',
    },
    {
      name: 'Exhaust',
      description: 'Manifolds, catalytic converters, DPF, silencers, downpipes',
    },
    {
      name: 'Fuel System',
      description: 'Injectors, fuel pumps, fuel rails, throttle bodies',
    },
  ];
  ...
  for (const m of modelsData) {
    ...
    let model = await modelRepo.findOne({
      where: { name: m.name, make: { id: makes[m.make].id } },
    });
    if (!model) {
  ...
  for (const g of gensData) {
    ...
    let gen = await genRepo.findOne({
      where: { code: g.code, model: { id: models[modelKey].id } },
    });
    if (!gen) {
      gen = await genRepo.save(
        genRepo.create({
          name: g.name,
          code: g.code,
  ...
  for (const v of variantsData) {
    ...
    const exists = await variantRepo.findOne({
      where: { name: v.name, generation: { id: gens[genKey].id } },
    });
    if (!exists) {
