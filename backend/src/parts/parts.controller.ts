...
export class PartsController {
  ...
  @Get() findAll(
    @Query('vehicleId') vehicleId?: string, @Query('categoryId') categoryId?: string,
    @Query('status') status?: string, @Query('condition') condition?: string,
    @Query('search') search?: string,
  ) {
    return this.svc.findAll({ vehicleId: vehicleId ? +vehicleId : undefined, categoryId: categoryId ? +categoryId : undefined, status, condition, search });
  }
